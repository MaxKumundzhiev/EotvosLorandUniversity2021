...
Now I can emphasize that each filter utilizes various functionalities described above. It can be either separate convolution operation within special filter or combination such as: Suppress noise, Compute gradient magnitude and direction, Apply non-maximum suppression, Hysteresis thresholding and Connectivity analysis to detect edges (Canny Edge Detector). Now I will show how Sobel filter performs on our baseline image.

\begin{figure}[h]  \includegraphics[width=7cm]{images/sample_vertebrae_sobel.jpeg}
    \caption {Applied Sobel filter for sample CT scan of vertebrae.}
    \label{fig:sobel}
\end{figure}


By the reason of edge detection is based on derivatives it can easily degrade with variations and noise. But issues which should be considered are size of the neighborhood as well as what exactly represents a change. 
